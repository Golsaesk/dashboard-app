...
export default function GoalCarousel({ goals = [], isLoading }: Props) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => scroll('left')}
        className="absolute top-1/2 z-10 -translate-y-1/2 rounded-full bg-white/80 p-1 shadow dark:bg-zinc-800"
      >
        <ChevronLeft />
      </button>
      ...
      <div
        ref={containerRef}
        className="scroll-snap-x flex snap-mandatory gap-4 overflow-x-auto scroll-smooth"
      >
        {goals.map((goal) => (
          <div
            key={goal.id}
            className="min-w-full rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900"
          >
            <GoalChart goal={goal} />
          </div>
        ))}
